...
void inclui_registro(lista * l, int valor){
	if(l == NULL) return;
...
void inclui_aresta(vertice * v, int valor){
	...
	inclui_registro(v->lista_adj, valor);
}
...
void dfs(vertice * v, int i, int dist){
	...
	if(v[i].lista_adj->inicio != NULL){
		registro * aux = v[i].lista_adj->inicio;
...
int main(){
	...
	printf("\nMaior distancia encontrada: %d -> %d : %d", maior, longe, ver[longe].distancia);
